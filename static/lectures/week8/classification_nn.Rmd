...
The task is to take a grid of $n \times n$ pixel values corresponding to the brightness of a number, and output a single number corresponding to the number the image represents.

Comically easy for us, not so trivial to code up! 
...
.orange[Idea:] capture a complex relationship between outputs and inputs by creating layers of derived variables.
