...
Linear discriminant analysis involves the inversion of a $p \times p$ within-covariance matrix. When $p \gg n$, this matrix can be huge, and has rank *at most* $n < p$, and hence is singular.

RDA overcomes this singularity by regularising the within-covariance matrix estimate $\hat{\boldsymbol{\Sigma}}$. It does this by shrinking $\hat{\boldsymbol{\Sigma}}$ towards its diagonal.

$$\hat{\boldsymbol{\Sigma}}(\gamma) = \gamma \hat{\boldsymbol{\Sigma}} + (1- \gamma) \mbox{diag}(\hat{\boldsymbol{\Sigma}}), \quad \mbox{with} \quad \gamma \in [0,1].$$
Note that $\gamma = 0$ corresponds to diagonal LDA.
---
